  ...
  switch (action.type) {
    case brandActionTypes.ADD_BRAND:
      ...
      let lastPaginationPage =
        Math.ceil(parseInt(state.totalRecords) / parseInt(state.rowsPerPage)) -
        1
      ...
      if (oldPaginations[lastPaginationPage]) {
        // if brand already clicked on last page
        ...
        if (
          lastPaginationEndIndex - lastPaginationStartIndex ==
          state.rowsPerPage
        ) {
          // if last page has more than rowsPerPage Records, than add new page to pagination array
          updatedPaginationCurrentPage = lastPaginationPage + 1
          oldPaginations[lastPaginationPage + 1] = {
            startIndex: lastPaginationEndIndex,
            endIndex: lastPaginationEndIndex + 1
          }
        } else {
          // update the lastPaginatinEndIndex to get the new record added
          oldPaginations[lastPaginationPage] = {
            startIndex: lastPaginationStartIndex,
            endIndex: lastPaginationEndIndex + 1
          }
        }
      }
    ...
    case brandActionTypes.DELETE_BRAND:
      ...
      let brandsForDeletedRecordPage = state.brands.slice(
        state.paginationArray[action.payload.page].startIndex,
        state.paginationArray[action.payload.page].endIndex
      )
      const deletedRecordIndex = brandsForDeletedRecordPage.findIndex(
    ...
    case brandActionTypes.BRANDS_LOADED:
      ...
      if (action.payload.brands) {
        let newPageRecord = {
          startIndex: state.brands.length,
          endIndex: updatedBrandsArray.length
        }
    ...
    case brandActionTypes.ALL_BRANDS_LOADED:
      return {
        ...state,
        allBrands: [...action.payload, state.allBrands],
