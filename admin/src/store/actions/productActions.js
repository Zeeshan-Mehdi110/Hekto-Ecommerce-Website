...
export const loadProducts = (
  ...
  return (dispatch, getState) => {
    ...
    if (state.products.allRecordsLoaded)
      // don't send request again and again if all records have loaded
      return;

    let skipRecords = 0;
    if (state.products.products.length === 0) dispatch(showProgressBar());
    ...
    axios
      ...
      .then(({ data }) => {
        const state = getState();
        if (state.products.products.length === 0) dispatch(hideProgressBar());

        const allRecordsLoaded =
          state.products.products.length + data.products.length ===
          data.totalRecords;
        dispatch({
          type: productActionTypes.PRODUCTS_LOADED,
          payload: {
        ...
        dispatch({
          type: productActionTypes.UPDATE_PAGINATION_CURRENT_PAGE,
          payload: currentPage,
        });
      })
      .catch((err) => {
