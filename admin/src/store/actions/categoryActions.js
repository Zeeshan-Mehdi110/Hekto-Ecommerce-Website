...
export const loadCategories = (
  ...
  return (dispatch, getState) => {
    ...
    if (state.categories.allRecordsLoaded)
      // don't send request again and again if all records have loaded
      return;

    let skipRecords = 0;
    if (state.categories.categories.length === 0) dispatch(showProgressBar());
    ...
    axios
      ...
      .then(({ data }) => {
        ...
        const allRecordsLoaded =
          state.categories.categories.length + data.categories.length ===
          data.totalRecords;
        dispatch({
          type: categoryActionTypes.CATEGORIES_LIST,
          payload: {
        ...
        dispatch({
          type: categoryActionTypes.UPDATE_PAGINATION_CURRENT_PAGE,
          payload: currentPage,
        });
      })
      .catch((err) => {
