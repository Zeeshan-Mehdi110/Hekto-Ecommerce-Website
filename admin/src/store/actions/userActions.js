...
export const userActionTypes = {
  ...
  UPDATE_ROWS_PREPARE: "UPDATE_ROWS_PREPARE",
...
export const loadUsers = (
  currentPage = 1,
  recordsPerPage = process.env.REACT_APP_RECORDS_PER_PAGE
) => {
  return (dispatch, getState) => {
    const state = getState();

    if (state.users.allRecordsLoaded)
      // don't send request again and again if all records have loaded
      return;

    let skipRecords = 0;
    if (state.users.users.length === 0) dispatch(showProgressBar());

    skipRecords = parseInt(currentPage) * parseInt(recordsPerPage);

    axios
      .get("/users", { params: { skip: skipRecords, limit: recordsPerPage } })
      .then(({ data }) => {
        const state = getState();
        if (state.users.users.length === 0) dispatch(hideProgressBar());

        const allRecordsLoaded =
          state.users.users.length + data.users.length === data.totalRecords;
        dispatch({
        ...
        dispatch({
          type: userActionTypes.UPDATE_PAGINATION_CURRENT_PAGE,
          payload: currentPage,
        });
      })
      .catch((err) => {
