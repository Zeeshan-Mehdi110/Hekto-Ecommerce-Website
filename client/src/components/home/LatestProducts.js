...
function LatestProducts() {
  ...
  const { latestProducts } = useSelector(({ home }) => home.data || {});

  if (!latestProducts) {
    return null; // Render nothing if latestProducts is null
  }

  const categories = Object.keys(latestProducts);

  if (categories.length === 0) {
    return null; // Render nothing if categories are empty
  }
          ...
          <div key={index} role="tabpanel" hidden={value !== index} aria-labelledby={`simple-tab-${index}`}>
            <Box sx={{ p: !isMobile ? 3 : 0 }}>
              <Grid container columnSpacing={1}>
                {Array.isArray(latestProducts[category]) ? (
