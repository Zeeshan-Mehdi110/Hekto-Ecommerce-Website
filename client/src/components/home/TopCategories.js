...
function TopCategories() {
  ...
  if (!topProducts || topProducts.length === 0) {
    return null; // Render nothing if topProducts is null or empty
  }
  ...
  return (
    <>
      <Box mt={5} textAlign="center">
        <Typography
          variant="h1"
          sx={{ ...themeStyles.mainHeading, color: "#151875", fontSize: !isMobile ? "42px" : "32px" }}
        >
          Top Categories
        </Typography>
      </Box>
      ...
      <Slider key={isMobile ? "mobile" : "desktop"} {...settings}>
        {topProducts.map((product) => (
          <ShopProductCard
            ...
            paperHoverStyle={{
              border: "0",
              "&:hover .detailsButton": { visibility: "visible" },
            }}
            imageBoxStyle={{
              backgroundColor: "#F6F7FB",
              boxShadow: "0px 8px 40px 0px #31208A0D",
            ...
            imageBoxContent={
              <Box position="absolute" bottom={0} py={1} visibility="hidden" className="detailsButton" sx={{ transform: "rotate(54deg) translateX(-60%)" }}>
                <Button
                  variant="contained"
                  sx={{
                    padding: "5px 12px",
                    ...
                    fontSize: "12px",
                    "&:hover": { backgroundColor: "#08D15F", color: "#FFFFFF" },
                  }}
                  size="small"
                >
                  View Details
